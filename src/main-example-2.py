            ...
            if "3.1" in title:
                ...
                if dates:
                    try:
                        ...
                        end_date = datetime.strptime(dates[1].text.split(" по ")[1], "%d.%м.%Y")
...
def main():
    # Пути к файлам
    contract_pdf_path = "docs/example-2/contract_6910851.pdf"
    contract_xml_path = "docs/example-2/contract_6910851.xml"
    word_doc_path = "docs/example-2/ТЗ 178_2023.docx"
    upd_files = [
        ("docs/example-1/UPD-589.pdf", "docs/example-2/UPD-589.xml"),
        ("docs/example-1/UPD-602.pdf", "docs/example-2/UPD-602.xml")
    ]
    ...
    for upd_pdf_path, upd_xml_path in upd_files:
        ...
        if contract_dates and submission_deadline and acceptance_dates:
            ...
            for result in results:
                print(f"Дата отгрузки: {result[0].strftime('%d.%m.%Y')}, Дата подписи: {result[1].strftime('%d.%м.%Y')}, Статус: {result[2]}")
        else:
